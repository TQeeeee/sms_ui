<template>
  <div class="chat-box">
    <header>发布通知 (在线:{{ count }}人)</header>
<!--    <div class="">班级群聊 (在线:{{ count }}人)</div>-->
    <div ref="msg-box" class="msg-box">
      ...
      >
        <div class="user-head">
          <img :src="item.avatar" height="30" width="30" :title="item.username">
        </div>
        <div class="user-msg">
          <span :style="item.userId == userId?' float: right;':''" :class="item.userId == userId?'right':'left'">{{ item.content }}</span>
        </div>
      </div>
    </div>
    <div class="input-box">
  ...
  methods: {
    ...
    initWebSocket() {
      ...
      if (window.WebSocket) {
        ...
        var url = 'ws://' + serverHot + ':5297' + '/groupChat/' + sip + '/' + this.userId // `ws://127.0.0.1/9101/groupChat/10086/聊天室`
